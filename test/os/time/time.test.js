...
describe('os.time', function() {
  ...
  it('should apply offsets to dates', function() {
    ...
    expect(os.time.getTimeOffset()).toBe(4 * 60 * 60 * 1000 + 15 * 60 * 1000);
    expect(os.time.getTimeOffsetLabel()).toBe('+0415');
    expect(os.time.toOffsetString(0)).toBe('1970-01-01T04:15:00 +0415');

    os.time.applyOffset('-0600');
    expect(os.time.getTimeOffset()).toBe(-6 * 60 * 60 * 1000);
    expect(os.time.getTimeOffsetLabel()).toBe('-0600');
