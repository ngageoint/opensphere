goog.provide('os.state.v3.LayerState');
goog.provide('os.state.v3.LayerTag');
 ...
 * @constructor
 */
os.state.v3.LayerState = function() {
  os.state.v3.LayerState.base(this, 'constructor');
   ...
   * @protected
   */
  this.logger = os.state.v3.LayerState.LOGGER_;
};
goog.inherits(os.state.v3.LayerState, os.state.XMLState);
 ...
 * @const
 */
os.state.v3.LayerState.LOGGER_ = goog.log.getLogger('os.state.v3.LayerState');
 ...
 * @protected
 */
os.state.v3.LayerState.prototype.hasLocalData = function(layerOptions) {
 ...
 * @protected
 */
os.state.v3.LayerState.prototype.isValid = function(layer) {
  try {
 ...
 * @inheritDoc
 */
os.state.v3.LayerState.prototype.load = function(obj, id) {
  obj = os.state.XMLState.ensureXML(obj);

  if (!(obj instanceof Element)) {
    goog.log.error(os.state.v3.LayerState.LOGGER_, 'Unable to load state content!');
    return;
  }
  ...
  try {
    ...
    // que pasa, hombre?
    goog.log.error(os.state.v3.LayerState.LOGGER_, 'Error loading layer in state ' + id, e);
  }
};
 ...
 * @inheritDoc
 */
os.state.v3.LayerState.prototype.remove = function(id) {
 ...
 * @inheritDoc
 */
os.state.v3.LayerState.prototype.saveInternal = function(options, rootObj) {
  try {
    var layers = os.MapContainer.getInstance().getLayers();
    var hasLocked = false;
    for (var i = 0, n = layers.length; i < n; i++) {
 ...
 * @protected
 */
os.state.v3.LayerState.prototype.layerToXML = function(layer, options, opt_exclusions, opt_layerConfig) {
  ...
  var bfs = type != 'wms' ? os.xml.createElement(os.state.v3.LayerTag.BFS, options.doc) : null;
    ...
    if (goog.isDefAndNotNull(value)) {
      switch (key) {
        case 'params':
          ...
          break;
        case 'provider':
          os.xml.appendElement(os.state.v3.LayerTag.DATA_PROVIDER, layerEl, value);
          break;
        case 'baseType':
          // include the provider tag
          os.xml.appendElement(os.state.v3.LayerTag.PROVIDER_TYPE, layerEl, value);
        ...
        case os.style.StyleField.COLOR:
          if (bfs) {
            ...
            os.xml.appendElement(os.state.v3.LayerTag.PT_COLOR, bfs, pointColor);
          } else {
            // tile layer
            this.defaultConfigToXML(key, value, layerEl);
          }
        ...
        case 'contrast':
          if (goog.isNumber(value) && !isNaN(value)) {
            ...
            os.xml.appendElement(os.state.v3.LayerTag.CONTRAST, layerEl, Math.round((value - 1) * 100));
          }
          break;
        case 'alpha':
        case 'opacity':
        ...
        case 'size':
          ...
          break;
        case 'styles':
          var stylesEl = os.xml.appendElement(os.state.v3.LayerTag.STYLES, layerEl);
        ...
        case os.style.StyleField.LABELS:
          ...
          if (bfs && labelColumn.length > 0 &&
            ...
            // New Multi column
            var labelColumns = os.xml.appendElement(os.state.v3.LayerTag.LABEL_COLUMNS, bfs);
            goog.array.forEach(labelColumn, function(label) {
              os.xml.appendElement(os.state.v3.LayerTag.LABEL, labelColumns, undefined, {
                'column': label['column'],
        ...
        case os.style.StyleField.LABEL_SIZE:
          if (bfs && goog.isNumber(value)) {
            os.xml.appendElement(os.state.v3.LayerTag.LABEL_SIZE, bfs, value);
          }
          break;
        case os.style.StyleField.SHOW_LABELS:
          os.xml.appendElement(os.state.v3.LayerTag.SHOW_LABELS, layerEl, value);
          break;
        case 'mappings':
          var mappings = /** @type {Array.<os.im.mapping.IMapping>} */ (layerConfig['mappings']);
          if (mappings && mappings.length > 0) {
            var mappingsStateEl = os.xml.appendElement(os.state.v3.LayerTag.MAPPINGS, layerEl);
          ...
          break;
        case os.state.v3.LayerTag.CSV_COMMENT_CHAR:
        ...
        case os.state.v3.LayerTag.CSV_HEADER_ROW:
          ...
          break;
        case os.state.v3.LayerTag.CSV_USE_HEADER:
        case os.state.v3.LayerTag.PARSER_CONFIG:
          // NO-OP
          break;
        case os.state.v3.LayerTag.ANIMATE:
          ...
          if (!(os.state.v3.LayerTag.TIME_ENABLED in layerConfig)) {
            this.defaultConfigToXML(key, value, layerEl);
          }
          break;
        case os.state.v3.LayerTag.TIME_ENABLED:
          // save under the animate element
          this.defaultConfigToXML(os.state.v3.LayerTag.ANIMATE, value, layerEl);
          break;
        case os.state.v3.LayerTag.IS_BASE_LAYER:
          this.defaultConfigToXML('baseLayer', value, layerEl);
          break;
        case 'refreshInterval':
          os.xml.appendElement(os.state.v3.LayerTag.REFRESH_RATE, layerEl, value);
          break;
        default:
          this.defaultConfigToXML(key, value, layerEl);
          break;
  ...
  if (type == 'wms' && url) {
    os.xml.appendElement(os.state.v3.LayerTag.MAP_URL, layerEl, url);
  }
 ...
 * @protected
 */
os.state.v3.LayerState.prototype.defaultConfigToXML = function(key, value, layerEl) {
  var node = null;
  ...
  if (goog.isString(value) || goog.isNumber(value) || goog.isBoolean(value)) {
    ...
    // classes
    var persistObj = os.ui.stateManager.getPersistable(key);
    if (persistObj) {
 ...
 * @protected
 */
os.state.v3.LayerState.prototype.defaultXmlToConfig = function(key, el) {
  var result = null;
  var persistObj = os.ui.stateManager.getPersistable(key);
 ...
 * @protected
 */
os.state.v3.LayerState.prototype.analyzeOptions = function(options, id) {
 ...
 * @protected
 */
os.state.v3.LayerState.prototype.xmlToOptions = function(node) {
  if (node) {
        ...
        switch (name) {
          case os.state.v3.LayerTag.BFS:
            ...
            for (var j = 0, m = styleList.length; j < m; j++) {
              ...
              switch (styleName) {
                case os.state.v3.LayerTag.PT_COLOR:
                  try {
                ...
                case os.state.v3.LayerTag.LABEL_COLUMNS:
                  ...
                  break;
                case os.state.v3.LayerTag.LABEL_COLOR:
                  try {
                    styleVal = os.color.padHexColor(styleVal, '#');
                  ...
                  break;
                case os.state.v3.LayerTag.LABEL_SIZE:
            ...
            break;
          case os.state.v3.LayerTag.CONTRAST:
            ...
            break;
          case os.state.v3.LayerTag.DATA_PROVIDER:
            options['provider'] = child.textContent;
            break;
          case os.state.v3.LayerTag.MAP_URL:
            // trim trailing ? from the url
            options['url'] = child.textContent.replace(/[?]+$/, '');
            break;
          case os.state.v3.LayerTag.PARAMS:
            ...
            options['params'] = qd;
            break;
          case os.state.v3.LayerTag.STYLES:
            ...
            break;
          case os.state.v3.LayerTag.TAGS:
            options['tags'] = os.tag.tagsFromXML(child);
            break;
          case os.state.v3.LayerTag.TITLE:
            ...
            break;
          case os.state.v3.LayerTag.SHOW_LABELS:
            options[os.style.StyleField.SHOW_LABELS] = child.textContent == 'true';
            break;
          case os.state.v3.LayerTag.MAPPINGS:
          ...
          case os.state.v3.LayerTag.ANIMATE:
            ...
            options[os.state.v3.LayerTag.TIME_ENABLED] = options[name];
            break;
          case os.state.v3.LayerTag.TIME_ENABLED:
            // NO-OP (handled by the animate element)
            break;
          case 'comment':
            options[os.state.v3.LayerTag.CSV_COMMENT_CHAR] = this.defaultXmlToConfig(name, child);
            break;
          case os.state.v3.LayerTag.CSV_DATA_ROW:
            // convert from 0 based indexing to 1 based indexing.
            options[os.state.v3.LayerTag.CSV_DATA_ROW] = this.defaultXmlToConfig(name, child) + 1;
            break;
          case os.state.v3.LayerTag.CSV_HEADER_ROW:
            // convert from 0 based indexing to 1 based indexing.
            var val = /** @type {number} */ (this.defaultXmlToConfig(name, child));
            options[os.state.v3.LayerTag.CSV_USE_HEADER] = val < 0 ? false : true;
            options[os.state.v3.LayerTag.CSV_HEADER_ROW] = val + 1;
            break;
          case 'baseLayer':
            options[os.state.v3.LayerTag.IS_BASE_LAYER] = this.defaultXmlToConfig(name, child);
            break;
          case os.state.v3.LayerTag.REFRESH_RATE:
