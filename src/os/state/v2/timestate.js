goog.provide('os.state.v2.TimeState');
goog.provide('os.state.v2.TimeTag');
 ...
 * @constructor
 */
os.state.v2.TimeState = function() {
  os.state.v2.TimeState.base(this, 'constructor');
  this.description = 'Saves the current timeline';
  this.priority = 100;
  this.rootName = os.state.v2.TimeTag.TIME;
  this.title = 'Time';
};
goog.inherits(os.state.v2.TimeState, os.state.XMLState);
 ...
 * @const
 */
os.state.v2.TimeState.LOGGER_ = goog.log.getLogger('os.state.v2.TimeState');
 ...
 * @const
 */
os.state.v2.TimeState.DATE_FORMAT = 'YYYY-MM-DDTHH:mm:ss[Z]';


/**
 * @inheritDoc
 */
os.state.v2.TimeState.prototype.load = function(obj, id) {
  obj = os.state.XMLState.ensureXML(obj);

  if (!(obj instanceof Element)) {
    goog.log.error(os.state.v2.TimeState.LOGGER_, 'Unable to load state content!');
    return;
  }

  try {
    if (!os.state.v2.TimeState.testUIState(obj)) {
      ...
      var loadDelay = new goog.async.ConditionalDelay(os.state.v2.TimeState.testUIState.bind(undefined, obj));
    ...
    goog.log.error(os.state.v2.TimeState.LOGGER_, 'Failed to parse time state! Stack: ' + e.stack);
  }
};
 ...
 * @protected
 */
os.state.v2.TimeState.prototype.loadInternal = function(obj, id) {
  try {
    var animationEl = obj.querySelector(os.state.v2.TimeTag.ANIMATION);
    var tActive = animationEl != null && animationEl.childElementCount > 0;
    ...
    if (fullTimeLineRange) {
      ...
      var animateRanges = this.readIntervalsAsRangeSet_(obj, os.state.v2.TimeTag.SEQUENCE);
      ...
      tlc.setHoldRanges(this.readIntervalsAsRangeSet_(obj, os.state.v2.TimeTag.HOLDS));
    ...
    if (tActive) {
      ...
      var stateEl = animationEl.querySelector(os.state.v2.TimeTag.PLAY_STATE);
 ...
 * @inheritDoc
 */
os.state.v2.TimeState.prototype.remove = function(id) {
  // do nothing
};
 ...
 * @inheritDoc
 */
os.state.v2.TimeState.prototype.saveInternal = function(options, rootObj) {
  try {
    var tlc = os.time.TimelineController.getInstance();
    ...
    os.xml.appendElement(os.state.v2.TimeTag.CURRENT, rootObj,
        this.rangeToDateFormatString_(currentRange));
    os.xml.appendElement(os.state.v2.TimeTag.ADVANCE, rootObj, advance);
    ...
    if (tlc.hasAnimationRanges()) {
      ...
      var sequence = /** @type {!Element} */ (os.xml.appendElement(os.state.v2.TimeTag.SEQUENCE, rootObj));
      this.addRanges_(tlc.getAnimationRanges(), sequence);
    }

    if (tlc.hasHoldRanges()) {
      var holds = /** @type {!Element} */ (os.xml.appendElement(os.state.v2.TimeTag.HOLDS, rootObj));
    ...
    if (goog.isDefAndNotNull(timeline)) {
      ...
      var animation = os.xml.appendElement(os.state.v2.TimeTag.ANIMATION, rootObj);
      os.xml.appendElement(os.state.v2.TimeTag.LOOP, animation, loop);
      os.xml.appendElement(os.state.v2.TimeTag.LOOP_BEHAVIOR, animation, loopBehavior);
      os.xml.appendElement(os.state.v2.TimeTag.MS_PER_FRAME, animation, millisPerFrame);
      os.xml.appendElement(os.state.v2.TimeTag.PLAY_STATE, animation, playState);
    }

    os.xml.appendElement(os.state.v2.TimeTag.DURATION, rootObj, tlc.getDuration());
 ...
 * @private
 */
os.state.v2.TimeState.prototype.addRanges_ = function(timeranges, container) {
  var dateString;
  var range;
  ...
  for (var i = 0; i < timeranges.length; i = i + 1) {
    ...
    os.xml.appendElement(os.state.v2.TimeTag.SEQ_INTERVAL, container, dateString);
  }
};
 ...
 * @private
 */
os.state.v2.TimeState.prototype.rangeToDateFormatString_ = function(range) {
  var startDate = new Date(range.start);
  var endDate = new Date(range.end);
  return os.time.momentFormat(startDate, os.state.v2.TimeState.DATE_FORMAT, true) +
      '/' + os.time.momentFormat(endDate, os.state.v2.TimeState.DATE_FORMAT, true);
};
 ...
 * @private
 */
os.state.v2.TimeState.prototype.intervalStringToRange_ = function(interval) {
  var parts = interval.split(/\//);
  var startDate = os.time.parseMoment(parts[0], [os.state.v2.TimeState.DATE_FORMAT], true);
  var endDate = os.time.parseMoment(parts[1], [os.state.v2.TimeState.DATE_FORMAT], true);
 ...
 * @private
 */
os.state.v2.TimeState.prototype.readRangeFromIntervals_ = function(element) {
  var range = null;

  // Create range from all interval elements, as desktop does not include a root interval.
  var intervalElements = element.querySelectorAll(os.state.v2.TimeTag.INTERVAL);
 ...
 * @private
 */
os.state.v2.TimeState.prototype.readDuration_ = function(element, range) {
  var durationElement = element.querySelector(os.state.v2.TimeTag.DURATION);
  if (durationElement) {
 ...
 * @private
 */
os.state.v2.TimeState.prototype.readIntervalsAsRangeSet_ = function(element, tag) {
  var rangeSet = new goog.math.RangeSet();
  if (element) {
    var parentElement = element.querySelector(tag);
    if (parentElement) {
      var intervals = parentElement.querySelectorAll(os.state.v2.TimeTag.INTERVAL);
 ...
 * @private
 */
os.state.v2.TimeState.prototype.readFps_ = function(element) {
  var result = 0;
  var msPerFrameEl = element.querySelector(os.state.v2.TimeTag.MS_PER_FRAME);
 ...
 * @private
 */
os.state.v2.TimeState.prototype.readCurrent_ = function(element) {
  var currentEl = element.querySelector(os.state.v2.TimeTag.CURRENT);
 ...
 * @protected
 */
os.state.v2.TimeState.prototype.parsePeriod = function(period) {
  if (period) {
