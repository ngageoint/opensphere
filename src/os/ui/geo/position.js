 ...
 * {@link ui.geo.PositionEventType.MAP_CLICK} events to be $broadcast on the scope. These events should be fired
 ...
 */
Module.directive('position', [directive]);
...
class Controller {
  ...
  onPosText_() {
    ...
    if (text) {
      var result = geo.parseLatLon(text, this.scope_['order']);
  ...
  onCoord_() {
    if (!isNaN(this.scope_['geom']['lat']) && !isNaN(this.scope_['geom']['lon'])) {
      if (this.scope_['posText']) {
        var result = geo.parseLatLon(this.scope_['posText'], this.scope_['order']);
        if (result != null) {
          if (Math.abs(result.lat - this.scope_['geom']['lat']) > geo.EPSILON ||
              Math.abs(result.lon - this.scope_['geom']['lon']) > geo.EPSILON) {
            this.formatLatLon_();
          }
        }
      } else {
