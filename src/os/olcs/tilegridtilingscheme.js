goog.provide('os.olcs.TileGridTilingScheme');
 ...
 * @constructor
 */
os.olcs.TileGridTilingScheme = function(source) {
 ...
 */
os.olcs.TileGridTilingScheme.init = function() {
  /**
   * @param {!ol.source.TileImage} source
   * @extends {Cesium.TilingScheme}
   * @constructor
  ...
  os.olcs.TileGridTilingScheme = function(source) {
     ...
     * @private
     */
    this.projectedRect_ = /** @type {!Cesium.Rectangle} */ (this.rectangleToNativeRectangle(this.rectangle_));
  };
  goog.inherits(os.olcs.TileGridTilingScheme, Cesium.TilingScheme);

  // after creating the class, ensure this function can still be called without consequence.
  os.olcs.TileGridTilingScheme.init = goog.nullFunction;


  Object.defineProperties(os.olcs.TileGridTilingScheme.prototype, {
    'ellipsoid': {
      get:
        /**
         * @return {!Cesium.Ellipsoid}
         * @this os.olcs.TileGridTilingScheme
         */
        function() {
   ...
   * @inheritDoc
   */
  os.olcs.TileGridTilingScheme.prototype.getNumberOfXTilesAtLevel = function(level) {
   ...
   * @inheritDoc
   */
  os.olcs.TileGridTilingScheme.prototype.getNumberOfYTilesAtLevel = function(level) {
    var tileRange = this.tilegrid_.getFullTileRange(level);
    if (tileRange) {
   ...
   * @inheritDoc
   */
  os.olcs.TileGridTilingScheme.prototype.rectangleToNativeRectangle = function(rectangle, opt_result) {
   ...
   * @inheritDoc
   */
  os.olcs.TileGridTilingScheme.prototype.tileXYToNativeRectangle = function(x, y, level, opt_result) {
   ...
   * @inheritDoc
   */
  os.olcs.TileGridTilingScheme.prototype.tileXYToRectangle = function(x, y, level, opt_result) {
   ...
   */
  os.olcs.TileGridTilingScheme.prototype.positionToTileXY = function(position, level, opt_result) {
    ...
    var x = Math.floor((coord[0] - origin[0]) / resolution) / tileSize[0];
    var y = Math.floor((origin[1] - coord[1]) / resolution) / tileSize[1];

    var result = opt_result || new Cesium.Cartesian2();
    result.x = Math.floor(x);
    result.y = Math.floor(y);

    console.log('tile coord at level', level, 'for position', coord, 'is', result);
    return result;
  };
   ...
   */
  os.olcs.TileGridTilingScheme.prototype.contains = function(position) {
    var epsilon = 1E-12;
