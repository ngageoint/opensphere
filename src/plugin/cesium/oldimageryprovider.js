goog.provide('plugin.cesium.OldImageryProvider');
 ...
 * @constructor
 */
plugin.cesium.OldImageryProvider = function(source, opt_fallbackProj) {
  /**
   * @type {boolean}
   * @private
   */
 ...
 * @inheritDoc
 */
plugin.cesium.OldImageryProvider.prototype.dispose = function() {
  this.disposed_ = true;
 ...
 * @inheritDoc
 */
plugin.cesium.OldImageryProvider.prototype.isDisposed = function() {
 ...
 * @private
 */
plugin.cesium.OldImageryProvider.prototype.onSourceChange_ = function(opt_event) {
  if (!this.ready_ && this.source.getState() == 'ready') {
    ...
    this.credit_ = plugin.cesium.OldImageryProvider.createCreditForSource(this.source) || undefined;
    this.ready_ = true;
  }
};
 ...
 * @override
 */
plugin.cesium.OldImageryProvider.prototype.getTileCredits = function(x, y, level) {
  return undefined;
};
goog.exportProperty(
    plugin.cesium.OldImageryProvider.prototype,
    'getTileCredits',
    plugin.cesium.OldImageryProvider.prototype.getTileCredits);
 ...
 * @protected
 */
plugin.cesium.OldImageryProvider.prototype.incrementLoading = function() {
  try {
 ...
 */
plugin.cesium.OldImageryProvider.prototype.decrementLoading = function(image) {
  try {
 ...
 * @suppress {accessControls}
 */
plugin.cesium.OldImageryProvider.prototype.decrementLoadingError = function(image) {
  try {
    if (this.source instanceof ol.source.UrlTile) {
 ...
 * @override
 */
plugin.cesium.OldImageryProvider.prototype.requestImage = function(x, y, level) {
        ...
        if (goog.isDef(promise)) {
          ...
          if (os.implements(this.source, os.source.IFilterableTileSource.ID)) {
            ...
            if (filterFns.length > 0) {
              ...
              var resolveBinding = plugin.cesium.OldImageryProvider.resolver.bind(this, filterFns, deferred);
...
goog.exportProperty(
    plugin.cesium.OldImageryProvider.prototype,
    'requestImage',
    plugin.cesium.OldImageryProvider.prototype.requestImage);
 ...
 */
plugin.cesium.OldImageryProvider.resolver = function(filterFns, deferred, image) {
  if (image) {
 ...
 */
plugin.cesium.OldImageryProvider.createCreditForSource = function(source) {
  var html;
...
Object.defineProperties(plugin.cesium.OldImageryProvider.prototype, {
  ready: {
    get:
        /**
         * @this plugin.cesium.OldImageryProvider
         * @return {boolean}
         */
        function() {
          return this.ready_;
