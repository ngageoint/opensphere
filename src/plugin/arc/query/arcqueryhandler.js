    ...
    for (var i = 0, ii = includeEntries.length; i < ii; i++) {
      ...
      if (inclusion) {
        try {
          ...
          log.error(ArcQueryHandler.LOGGER_, 'error merging inclusion areas', e);
        }
      }
    }
      ...
      for (var i = 0, ii = excludeEntries.length; i < ii; i++) {
        ...
        if (exclusion) {
          try {
            ...
            log.error(ArcQueryHandler.LOGGER_, 'error merging exclusion areas', e);
          }
        }
      }
    }
 ...
 * @const
 */
ArcQueryHandler.LOGGER_ = log.getLogger('plugin.arc.query.ArcQueryHandler');
